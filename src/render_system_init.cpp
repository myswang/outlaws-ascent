...
bool RenderSystem::initializeFonts() {
	...
	glm::mat4 projection = glm::mat4(createProjectionMatrix());
	...
	std::string font_filename = PROJECT_SOURCE_DIR + std::string("data/fonts/Kenney_High.ttf");
	FT_UInt font_default_size = 48;
...
void RenderSystem::initializeGlGeometryBuffers()
{
	...
	// Initialize 'bar' (for health and stamina)
	std::vector<ColoredVertex> bar_vertices;
	std::vector<uint16_t> bar_indices;

	constexpr float bar_depth = 1.0f;
	...
	// also 2 triangles
	bar_indices = { 0, 1, 2, 1, 3, 2 };

	geom_index = (int)GEOMETRY_BUFFER_ID::BAR;
	meshes[geom_index].vertices = bar_vertices;
	meshes[geom_index].vertex_indices = bar_indices;
	bindVBOandIBO(GEOMETRY_BUFFER_ID::BAR, bar_vertices, bar_indices);

	// Initialize "particle" vbos for instancing
	std::vector<ColoredVertex> part_vertices;
	...
	// also 2 triangles
	part_indices = { 0, 1, 2, 1, 3, 2 };

	geom_index = (int)GEOMETRY_BUFFER_ID::PARTICLE;
	meshes[geom_index].vertices = part_vertices;
	meshes[geom_index].vertex_indices = part_indices;
	bindVBOandIBO(GEOMETRY_BUFFER_ID::PARTICLE, part_vertices, part_indices);

	// CITATION: https://www.opengl-tutorial.org/intermediate-tutorials/billboards-particles/particles-instancing/
	// define arrays for transformation and colour instancing

	glGenVertexArrays(1, &m_part_VAO);

	glGenBuffers(1, &part_transform_buffer);
	gl_has_errors();

	glGenBuffers(1, &part_color_buffer);
	gl_has_errors();
